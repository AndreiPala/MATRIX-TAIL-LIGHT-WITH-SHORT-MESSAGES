...
const int font[][6] = {
  ...
  { 0x46, 0x49, 0xff, 0x49, 0x31, 0x00 },  // $ 0x24 36
...
int latch = A1;
int di = A0;
int columns[] = { 10, 11, 12, 13, A5, 5, 6, 7, 8, 9, 0, 2, 1, 3, 4 };
...
void Mprintln(String s) {
  ...
  j = 0;
  for (int i = 0; i < s.length() * 6 + 15; ++i) {
    for (int l = i; l < i + 15; ++l)
      if (l > 14 && l < s.length() * 6 + 15)
        arr[j++] = font[int(s[(l - 15) / 6]) - 32][(l - 15) % 6];
      else
        arr[j++] = font[0][0];
    j = 0;
    arraypush(arr);
